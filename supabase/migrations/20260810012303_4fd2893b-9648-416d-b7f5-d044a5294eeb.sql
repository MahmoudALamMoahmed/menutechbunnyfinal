...
BEGIN
  ...
  IF v_persist THEN
    INSERT INTO orders (
      ...
      concat_ws(' - ',
        CASE WHEN v_area.name IS NOT NULL THEN 'المنطقة: ' || v_area.name END,
        CASE WHEN v_branch.name IS NOT NULL THEN 'الفرع: ' || v_branch.name END,
        v_notes
      ),
